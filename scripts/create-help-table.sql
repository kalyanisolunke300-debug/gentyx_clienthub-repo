...
BEGIN
    ...
    (@AdminRoleId, 'Create and manage Client, CPA, and Service Center profiles', 1),
    (@AdminRoleId, 'Configure onboarding stages and associated tasks', 2),
    (@AdminRoleId, 'Assign CPA and Service Center to clients', 3),
    ...
    (@AdminRoleId, 'Assign Support Roles', 'Assign a CPA and Service Center to the client for compliance and operational support.', 'Users', 'action', 3),
    ...
    (@AdminRoleId, 'How do I reset a user''s password?', 'Go to the Client/CPA/Service Center list in the Dashboard, select the user, and click ''Edit''. You can set a new temporary password there. Users can also reset their password through the Settings page after logging in.', 1),
    ...
    (@AdminRoleId, 'How do I assign tasks to specific roles?', 'When creating or editing a task, use the ''Assigned To'' dropdown to select whether the task should be completed by the Client, CPA, or Service Center. Each role will only see their assigned tasks.', 4),
...
BEGIN
    ...
    (@ClientRoleId, 'Communicate with assigned CPA and Service Center', 3),
    ...
    (@ClientRoleId, 'Communicate', 'Use the messaging feature to contact your CPA or Service Center if you need help.', 'MessageCircle', 'action', 6),
    ...
    (@ClientRoleId, 'Can I edit a completed task?', 'Once a task is marked as completed, you cannot edit it directly. If you need to make changes, contact your CPA or Service Center to request the task be reopened.', 6),
    (@ClientRoleId, 'How do I contact my support team?', 'Use the Messages or Inbox feature to send secure messages to your assigned CPA or Service Center. They can help with questions, clarifications, or issues with your tasks.', 7);
...
BEGIN
    INSERT INTO dbo.help_roles (role_key, title, description, icon_name, color_class, display_order)
    VALUES ('CPA', 'CPA', 'Compliance and review role responsible for validating client tasks and documents. Ensure accuracy and regulatory compliance.', 'FileCheck', 'text-purple-600 dark:text-purple-400', 3);
    ...
    (@CpaRoleId, 'Upload documents for CPA-specific assigned tasks', 2),
    ...
    (@CpaRoleId, 'Review Tasks', 'Monitor client progress and review completed tasks requiring CPA validation.', 'ClipboardCheck', 'action', 3),
    (@CpaRoleId, 'Validate Documents', 'Check uploaded documents for accuracy, completeness, and compliance.', 'FileSearch', 'action', 4),
    (@CpaRoleId, 'Complete CPA Tasks', 'Execute tasks assigned specifically to the CPA role and upload required documents.', 'FilePlus', 'action', 5),
    ...
    (@CpaRoleId, 'Can I upload documents for the client?', 'Yes, CPAs can upload documents to client folders when necessary. This is typically done for CPA-specific tasks. Navigate to the Documents tab or the specific task assigned to CPA and use the upload feature.', 2),
    (@CpaRoleId, 'How do I reassign a task if needed?', 'Task reassignment must be done by an Admin. Contact your Admin if you believe a task should be assigned to a different role or another CPA.', 3),
    ...
    (@CpaRoleId, 'Can I communicate with other CPAs about a client?', 'Direct CPA-to-CPA messaging is not available in the system. For collaboration, contact your Admin who can facilitate information sharing or reassign clients as needed.', 6);
...
BEGIN
    ...
    (@ScRoleId, 'Coordinate with CPA for compliance matters', 5),
    ...
    (@ScRoleId, 'Can I move a client to the next stage?', 'Stage progression happens automatically once all required tasks in the current stage are completed. Ensure all tasks (including Client, CPA, and Service Center tasks) are checked off. If stages are still not progressing, contact Admin.', 2),
    ...
    (@ScRoleId, 'Can I see documents uploaded by other roles?', 'Yes, you can view all documents associated with your assigned clients through the Documents tab. This includes documents uploaded by the client, CPA, and other Service Center representatives.', 4),
    (@ScRoleId, 'How do I handle overdue tasks?', 'Filter your task view to show overdue items. Reach out to the responsible party (client for their tasks, coordinate with CPA for their tasks) to understand the delay and provide assistance.', 5),
